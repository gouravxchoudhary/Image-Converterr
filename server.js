...
// Serve static files from root directory with extension handling
...
// Blog routes moved to separate file for better organization
...
// Generic blog post handler for all blog posts
blogRouter.get('/:slug', (req, res) => {
  // Try to serve the HTML file for this slug
  const htmlFile = path.join(__dirname, 'blog', `${req.params.slug}.html`);
...
blogRouter.get('/:slug.html', (req, res) => {
...
app.post('/api/download-batch-zip', async (req, res) => {
  ...
  // Add files to ZIP with original names
  let fileCount = 0;
...
// These routes are now handled by the middleware above
...
// Redirect old .html FAQ URL to clean URL
